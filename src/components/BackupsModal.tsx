...
export default function BackupsModal({ onClose, onRestore, showToast }: BackupsModalProps) {
  const [backups, setBackups] = useState<Backup[] | null>(null);  // null = loading
  const [confirm, setConfirm] = useState<string | null>(null);    // day für Restore-Confirm
  ...
  const downloadJSON = async (day: string) => {
    try {
  ...
  return (
    <div role="dialog" aria-modal="true"
      ...
      onClick={(e: React.MouseEvent<HTMLDivElement>) => e.target === e.currentTarget && !acting && onClose?.()}>
