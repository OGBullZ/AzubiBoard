...
type ShareLink = { token: string; url: string; expires_at: string };
...
export default function ShareLinkModal({ kind, title, data, onClose }: ShareLinkModalProps) {
  const [ttl,     setTtl]     = useState(30);
  const [link,    setLink]    = useState<ShareLink | null>(null); // { token, url, expires_at }
  ...
  const create = async () => {
    ...
    try {
      ...
      setError((e as Error).message || 'Fehler beim Erstellen');
    } finally {
      setLoading(false);
    }
  };
