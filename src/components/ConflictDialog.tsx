...
type Snapshot = Partial<AppState> | null | undefined;
...
type DiffRow = { k: string; client: number; server: number };

type ConflictDialogProps = {
  payload: ConflictPayload;
  onAcceptServer: () => void | Promise<void>;
  onForceMine: () => void | Promise<void>;
  onReload: () => void | Promise<void>;
  onClose?: () => void;
};

function diffSummary(client: Snapshot, server: Snapshot): DiffRow[] | null {
  if (!client || !server) return null;
  const keys = ['projects', 'reports', 'users', 'groups', 'trainingPlan', 'calendarEvents', 'activityLog'] as const;
  const diffs: DiffRow[] = [];
  for (const k of keys) {
    const cv = (client as Record<string, unknown>)[k];
    const sv = (server as Record<string, unknown>)[k];
    const cn = Array.isArray(cv) ? cv.length : (cv ? 1 : 0);
    const sn = Array.isArray(sv) ? sv.length : (sv ? 1 : 0);
