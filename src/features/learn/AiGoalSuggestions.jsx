...
export default function AiGoalSuggestions({ lehrjahr, existingTitles = [], onAdd, onClose }) {
  ...
  const generate = async () => {
    ...
    setSelected(new Set());
    try {
      const result = await dataService.suggestGoals({
      ...
      setErr(e.message || 'Fehler beim Abrufen der Vorschläge');
    } finally {
      setLoading(false);
    }
  };
                ...
                <div key={i} onClick={() => toggle(i)} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: s.description ? 3 : 0 }}>
                      <span style={{ fontSize: 14, lineHeight: 1 }}>{TYPE_ICON[s.type] || '📋'}</span>
