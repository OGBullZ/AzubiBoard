...
// ── Hauptkomponente ───────────────────────────────────────────
export function PdfOcrImport({ existingFile, onImport, onClose }) {
  const [step,    setStep]    = useState('idle');   // idle | scanning | done | error
  const [msg,     setMsg]     = useState('');
  const [pct,     setPct]     = useState(0);
  const [result,  setResult]  = useState(null);     // { activities, learnings, raw }
  const [preview, setPreview] = useState('parsed'); // parsed | raw
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
              <div>
                ...
                <textarea value={result.activities}
                  onChange={e => setResult(r => ({ ...r, activities: e.target.value }))}
              ...
              <div>
                ...
                <textarea value={result.learnings}
                  onChange={e => setResult(r => ({ ...r, learnings: e.target.value }))}
