...
type TemplateKey = 'kaufmaennisch' | 'itDigital';
...
function parseJSON(text: string): ParseResult {
  let parsed: unknown;
  try { parsed = JSON.parse(text); } catch (e) { return { goals: [], errors: [`JSON ungültig: ${(e as Error).message}`] }; }
  const list: any[] | null = Array.isArray(parsed) ? parsed : (Array.isArray((parsed as any)?.goals) ? (parsed as any).goals : null);
...
type ImportGoalsModalProps = {
  existingGoals?: Array<{ title?: string; year?: number; quarter?: number }>;
  onImport: (goals: Goal[]) => void;
  onClose?: () => void;
};

export default function ImportGoalsModal({ existingGoals = [], onImport, onClose }: ImportGoalsModalProps) {
  const [tab, setTab]   = useState<Tab>('template'); // 'template' | 'csv' | 'json'
  const [text, setText] = useState('');
  const [tpl,  setTpl]  = useState<TemplateKey>('itDigital');
          ...
          {tab === 'template' && (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {(Object.entries(TEMPLATES) as Array<[TemplateKey, Template]>).map(([k, t]) => (
                  <label key={k} style={{
