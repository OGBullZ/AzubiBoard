...
// `assignees` ist im Runtime-Datenmodell vorhanden, aber (noch) nicht im
// Zod-Project-Schema typisiert — daher hier lokal ergänzt.
type ProjectWithAssignees = Project & { assignees?: Id[] };
type DeadlineItem = ProjectWithAssignees & { diff: number };

function DeadlineWidgetImpl({ projects, userId, isAusbilder, onOpen }: DeadlineWidgetProps) {
  const now = new Date();
