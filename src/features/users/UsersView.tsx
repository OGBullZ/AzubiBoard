...
// Laufzeit-Felder, die das User-Schema nicht abbildet (Passwort-Hash + Aktiv-Flags)
...
type UsersViewProps = {
  users: UserWithAuth[];
  onUpdateUsers: (users: UserWithAuth[]) => void;
  showToast: (msg: string) => void;
};

export function UsersView({ users, onUpdateUsers, showToast }: UsersViewProps) {
  const azubis = users.filter(u => u.role === 'azubi');
  const [showForm, setShowForm]     = useState(false);
  const [editUser, setEditUser]     = useState<UserWithAuth | null>(null);
  const [_confirmDel, _setConfirmDel] = useState<UserWithAuth | null>(null);
  ...
  const save = async () => {
    ...
    try {
      if (editUser) {
        ...
        if (USE_API) {
          // MySQL aktualisieren
          await dataService.updateUser(String(editUser.id), {
            name:                form.name.trim(),
      ...
      showToast('⚠ ' + ((e as Error).message || 'Unbekannter Fehler'));
    } finally {
      setSaving(false);
    }
  };

  const toggleActive = async (u: UserWithAuth) => {
    const activate = u.active === false || u.is_active === false;
    if (USE_API) {
      try {
        if (activate) await dataService.activateUser(String(u.id));
        else          await dataService.deactivateUser(String(u.id));
      } catch (e) {
        showToast('⚠ ' + ((e as Error).message || 'Fehler'));
        return;
      }
    }
  ...
  const isInactive = (u: UserWithAuth) => u.active === false || u.is_active === false;
