...
// ── Runtime-Shapes (divergieren vom strikten Zod-Schema) ──────
...
type GoalProgress = { status?: string };
...
type Goal = {
  ...
  progress?: Record<string, GoalProgress | undefined>;
};

type TrainingPlan = { goals?: Goal[] };

type ProfileData = {
  projects?: AzubiProject[];
  reports?: Report[];
  trainingPlan?: TrainingPlan;
};

type WeekDatum = { label: string; h: number };

// ── Mini Sparkline (Wochenstunden) ────────────────────────────
function HoursChart({ weeks }: { weeks: WeekDatum[] }) {
...
// ── Ring ──────────────────────────────────────────────────────
function Ring({ pct = 0, size = 56, stroke = 5, color }: { pct?: number; size?: number; stroke?: number; color?: string }) {
...
// ── Stat Box ──────────────────────────────────────────────────
function StatBox({ label, value, color, sub }: { label: string; value: React.ReactNode; color?: string; sub?: React.ReactNode }) {
  return (
...
// ── Main ──────────────────────────────────────────────────────
type AzubiProfilePageProps = {
  azubi: User | null | undefined;
...
export default function AzubiProfilePage({ azubi, data, currentUser: _currentUser, onBack }: AzubiProfilePageProps) {
  ...
  const weeksData = useMemo<WeekDatum[]>(() => {
    ...
    return Array.from({ length: 8 }, (_, i) => {
      ...
      const h  = allTasks.flatMap(t => (t.timeLog || []).filter(e => e.date != null && e.date >= ms && e.date <= me)).reduce((s, e) => s + (Number(e.hours) || 0), 0);
        ...
        <div className="card">
          ...
          {reports.length === 0
            ...
            : reports.slice(0, 10).map(r => {
                const ST: Record<string, [string, string]> = { draft: [C.mu,'Entwurf'], submitted: [C.ac,'Eingereicht'], reviewed: [C.yw,'Geprüft'], signed: [C.gr,'Unterschrieben'] };
                const [color, label] = ST[r.status ?? 'draft'] || ST.draft;
                return (
