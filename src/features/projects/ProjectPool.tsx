...
/**
 * Projekt-Objekte zur Laufzeit tragen mehr Felder als das strikte
 * Zod-Schema (assignees / groupId / links), die in dieser Ansicht
 * verwendet werden.
 */
type PoolProject = Project & {
  assignees?: Array<string | number>;
  groupId?: string | number;
  links?: unknown[];
};

type ProjectPoolProps = {
  ...
  onNew: () => void;
  onDelete: (id: Project['id'], title: Project['title']) => void;
  onArchive?: (id: Project['id']) => void;
  onUnarchive?: (id: Project['id']) => void;
  onDuplicate?: (id: Project['id']) => void;
};

type FilterValue = 'all' | 'mine' | 'green' | 'yellow' | 'red';
type SortValue = 'title_asc' | 'title_desc' | 'status' | 'deadline' | 'progress';
type FilterRow = [FilterValue, string, string | null, string, number];

export function ProjectPool({ projects, users, groups, currentUser, onOpen, onNew, onDelete, onArchive, onUnarchive, onDuplicate }: ProjectPoolProps) {
  ...
  const STATUS_ORDER: Record<string, number> = { green: 0, yellow: 1, red: 2 };
  ...
  const visible = active
    ...
    .sort((a, b) => {
      switch (sort) {
        ...
        case 'progress': {
          const pct = (p: PoolProject) => (p.tasks||[]).length === 0 ? 0 : (p.tasks||[]).filter(t => t.status === 'done' || t.done).length / (p.tasks||[]).length;
          return pct(b) - pct(a);
        }
        default: return 0;
      }
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: '16px 20px' }} className="anim">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, gap: 10, flexShrink: 0, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 7, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <select value={sort} onChange={e => setSort(e.target.value as SortValue)} aria-label={t('common.filter')}
