...
function LinkRow({ link, onRemove, readOnly }: LinkRowProps) {
  const t = LINK_TYPES[link.type as LinkType] || LINK_TYPES.other;
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', borderRadius: 7, transition: 'background .1s', background: hov ? 'var(--c-sf3)' : 'transparent' }}
      ...
      <div style={{ width: 20, height: 20, borderRadius: 4, background: t.color + '18', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
        {favicon
          ? <img src={favicon} width={12} height={12} alt="" onError={e => (e.target as HTMLImageElement).style.display = 'none'} />
...
export function LinksManager({ links = [], onUpdate, readOnly = false, compact: _compact = false }: LinksManagerProps) {
  ...
  const remove = (id: string) => onUpdate(links.filter(l => l.id !== id));
