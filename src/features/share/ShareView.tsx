...
// Read-Only-Report-Shape, wie er vom Share-Link-Backend geliefert wird.
// Bewusst lokal (nicht src/types Report), da Share-Daten zusätzliche Felder
// (user_name, reviewer_comment) enthalten, die nicht im strikten Schema sind.
type ShareReport = {
  id: string | number;
  week_start?: string;
...
export default function ShareView() {
  ...
  const [state, setState] = useState<ShareState>({ loading: true, data: null, error: null });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setState({ loading: false, data: null, error: (e as Error)?.message || 'Fehler' });
      }
    })();
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--c-bg)', padding: '24px 16px' }}>
      <div style={{ maxWidth: 880, margin: '0 auto' }}>
        ...
        {kind === 'bericht'     && <BerichtView data={data as ShareReport} title={title} />}
...
type JahresmappeViewProps = { data: unknown; title?: string | null };

function JahresmappeView({ data, title }: JahresmappeViewProps) {
  const reportsSource = (data as { reports?: unknown })?.reports;
  const reports: ShareReport[] = Array.isArray(reportsSource) ? reportsSource : [];
  return (
    <div>
...
type BerichtViewProps = { data: ShareReport; title?: string | null };

function BerichtView({ data, title }: BerichtViewProps) {
  return (
    <div>
...
type ReportCardProps = { r: ShareReport | null | undefined };

function ReportCard({ r }: ReportCardProps) {
  if (!r) return null;
