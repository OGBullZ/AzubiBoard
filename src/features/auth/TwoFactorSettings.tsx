...
type Stage = 'idle' | 'setup' | 'verify' | 'disable';

type TwoFactorSettingsProps = {
  showToast?: (msg: string) => void;
};

export default function TwoFactorSettings({ showToast }: TwoFactorSettingsProps) {
  const [status,  setStatus]  = useState<TwoFactorStatus | null>(null);   // null = loading
  const [stage,   setStage]   = useState<Stage>('idle'); // idle | setup | verify | disable
  const [secret,  setSecret]  = useState<string | null>(null);
  const [qrUrl,   setQrUrl]   = useState<string | null>(null);
  const [code,    setCode]    = useState('');
  const [pw,      setPw]      = useState('');
  const [recovery, setRecovery] = useState<string[] | null>(null); // Codes nach erfolgreichem Setup
  ...
  if (stage === 'disable') {
    return (
      <div style={{ padding: 14, background: C.sf2, border: `1px solid color-mix(in srgb, ${C.cr} 27%, transparent)`, borderRadius: 8 }}>
        ...
        <input type="password" value={pw} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPw(e.target.value)} placeholder="Passwort" autoComplete="current-password"
