...
describe('migrateData', () => {
  it('Brand-new data (v0) wird bis aktuelle Version migriert', () => {
    ...
    expect(next._migrationsApplied).toEqual([1, 2, 3, 4, 5]);
  ...
  it('v2 → v5: v3, v4 und v5 laufen', () => {
    ...
    expect(next.schema_version).toBe(CURRENT_SCHEMA_VERSION);
    expect(next._migrationsApplied).toEqual([3, 4, 5]);
...
describe('pendingMigrations', () => {
  it('listet alle ausstehenden Versionen', () => {
    expect(pendingMigrations({ schema_version: 0 })).toEqual([1, 2, 3, 4, 5]);
    expect(pendingMigrations({ schema_version: 2 })).toEqual([3, 4, 5]);
    expect(pendingMigrations({ schema_version: CURRENT_SCHEMA_VERSION })).toEqual([]);
  });
  it('ohne schema_version → ab v1', () => {
    expect(pendingMigrations({})).toEqual([1, 2, 3, 4, 5]);
  });
});
