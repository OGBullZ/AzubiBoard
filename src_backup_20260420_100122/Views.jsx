...
function Chip({ value, label, color, animated = false }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '6px 13px', background: color + '12', border: `1px solid ${color}25`, borderRadius: 8 }}>
      ...
      <span style={{ fontSize: 10, color: C.mu, fontWeight: 600, textTransform: 'uppercase', letterSpacing: .7, lineHeight: 1.3 }}>
        {label}
      </span>
    </div>
  );
...
function PanelTitle({ Icon, children, badge, action, onAction }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 11, flexShrink: 0 }}>
      {Icon && <Icon size={12} style={{ color: C.mu }} />}
      <span style={{ flex: 1, fontSize: 10, fontWeight: 700, color: C.mu, textTransform: 'uppercase', letterSpacing: 1 }}>{children}</span>
      {badge && (
...
function HeroTask({ task, onToggle, onOpen, onUpdateNote }) {
  ...
  if (!task) return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '20px 16px', borderRadius: 10, border: `1px solid ${C.gr}25`, background: C.gr + '08' }}>
      ...
      <div style={{ fontSize: 13, fontWeight: 700, color: C.gr }}>Alles erledigt!</div>
      <div style={{ fontSize: 11, color: C.mu, textAlign: 'center', lineHeight: 1.6 }}>Keine offenen oder überfälligen Aufgaben</div>
    </div>
  );
  ...
  return (
    <div style={{ borderRadius: 10, border: `1.5px solid ${accent}40`, background: accentBg, padding: '14px 15px', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
        ...
        <button onClick={onToggle}
          style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 7, border: `1.5px solid ${accent}`, background: 'transparent', color: accent, fontWeight: 700, fontSize: 11, cursor: 'pointer', transition: 'all .15s' }}
          ...
          <div key={t.id}
            ...
            <button onClick={() => onOpen(t.projectId)} style={{ flex: 1, minWidth: 0, background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', padding: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 1, alignItems: 'center' }}>
                <span style={{ fontSize: 10, color: C.mu, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 100 }}>{t.projectTitle}</span>
                ...
                {t.note && <IcoNote size={9} style={{ color: C.mu, flexShrink: 0 }} />}
              </div>
            </button>
          </div>
        );
    ...
    <div style={{ borderRadius: 10, border: `1px solid ${expanded ? sc + '60' : C.bd}`, background: C.sf2, transition: 'border-color .15s', marginBottom: 9, borderLeft: `3px solid ${sc}`, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 11px', cursor: 'pointer' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          ...
          <IcoChevronD size={11} style={{ color: C.mu, transition: 'transform .2s', transform: expanded ? 'rotate(180deg)' : 'none' }} />
        </div>
      </div>
      ...
      {total > 0 && (
        <div style={{ padding: '0 11px 9px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            <span style={{ fontSize: 9, color: C.mu, fontFamily: C.mono, flexShrink: 0 }}>{done}/{total}</span>
          </div>
        </div>
      )}
      ...
      {expanded && (
        <div style={{ borderTop: `1px solid ${C.bd}`, background: 'var(--c-sf3)', padding: '10px 11px' }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <span style={{ fontSize: 10, color: C.mu, fontWeight: 700, textTransform: 'uppercase', letterSpacing: .7 }}>Status:</span>
          ...
          {openTasks.length > 0 && (
            <div style={{ marginBottom: 9 }}>
              <div style={{ fontSize: 9, color: C.mu, textTransform: 'uppercase', letterSpacing: .7, fontWeight: 700, marginBottom: 5 }}>Offene Aufgaben</div>
              {openTasks.map(t => (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '4px 0', borderBottom: `1px solid ${C.bd}22` }}>
                  ...
                  <span style={{ fontSize: 11, color: C.tx, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {t.status === 'in_progress' && <span style={{ fontSize: 8, color: C.ac, marginRight: 3 }}>▶</span>}
                    {t.text}
                  </span>
                  {t.deadline && <span style={{ fontSize: 9, fontFamily: C.mono, color: C.mu, flexShrink: 0 }}>{fmtDate(t.deadline)}</span>}
                </div>
              ))}
          ...
          {onUpdate && (
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 8 }}>
              <input ref={inputRef}
                ...
                style={{ flex: 1, fontSize: 11, padding: '5px 9px', background: 'var(--c-sf)', border: `1px solid ${C.bd2}` }} />
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        <span style={{ fontSize: 11, fontFamily: C.mono, color: weekDone === weekTotal && weekTotal > 0 ? C.gr : C.ac, fontWeight: 700 }}>
...
function DeadlineWidget({ projects, userId, isAusbilder, onOpen }) {
  ...
  if (!items.length) return (
    <div style={{ fontSize: 11, color: C.mu, textAlign: 'center', padding: '10px 0' }}>
  ...
  const groups = [
    ...
    { label: 'Nächste Woche',      items: items.filter(i => i.diff > 7 && i.diff <= 14), color: C.mu },
    { label: 'Später',             items: items.filter(i => i.diff > 14),                color: C.mu },
        ...
        <div key={g.label}>
          ...
          {g.items.map(p => (
            <button key={p.id} onClick={() => onOpen(p.id)} className="row-btn"
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: C.br, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.title}</div>
              </div>
...
function CalWidget({ calendarEvents, projects, onNavigate }) {
  ...
  return (
    <div>
      {all.length === 0
        ? <div style={{ fontSize: 11, color: C.mu, textAlign: 'center', padding: '8px 0' }}>Keine Termine in Sicht</div>
        : all.map(e => {
            ...
            return (
              <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '5px 0', borderBottom: `1px solid var(--c-bd)` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 10, color: C.mu }}>{fmtDate(e.date)}</div>
                </div>
      ...
      {thisWeekMissing && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', background: C.ywd, border: `1px solid ${C.yw}30`, borderRadius: 7, marginBottom: 9 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 10, color: C.mu }}>Noch nicht für KW {Math.ceil((now - new Date(now.getFullYear(),0,1))/604800000)} erstellt</div>
          </div>
        </div>
      )}
      {total > 0 ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 5, marginBottom: 9 }}>
          {[['Entwurf', draft, C.mu], ['Eingereicht', sub, C.ac], ['Fertig', signed, C.gr]].map(([l, v, c]) => (
            <div key={l} style={{ textAlign: 'center', padding: '5px 3px', background: c + '10', borderRadius: 6, border: `1px solid ${c}20` }}>
              ...
              <div style={{ fontSize: 9, color: C.mu, marginTop: 2, textTransform: 'uppercase', letterSpacing: .5 }}>{l}</div>
            </div>
          ))}
        </div>
      ) : (
        <div style={{ fontSize: 11, color: C.mu, textAlign: 'center', padding: '4px 0 8px', fontStyle: 'italic' }}>Noch keine Berichtshefte</div>
      )}
...
function LearnWidget({ userId, onNavigate }) {
  ...
  if (history.length === 0) {
    return (
      <div>
        <div style={{ fontSize: 12, color: C.mu, fontStyle: 'italic', marginBottom: 9 }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7, marginBottom: 9 }}>
        {[
          ...
          <div key={s.l} style={{ background: 'var(--c-sf3)', borderRadius: 7, padding: '6px 9px', border: `1px solid var(--c-bd)` }}>
            <div style={{ fontSize: 9, color: C.mu, textTransform: 'uppercase', letterSpacing: .7, fontWeight: 700 }}>{s.l}</div>
      ...
      {last && (
        <div style={{ marginBottom: 9 }}>
          <div style={{ fontSize: 10, color: C.mu, marginBottom: 4 }}>
          ...
          <div style={{ fontSize: 10, fontFamily: C.mono, color: C.mu, marginTop: 3, textAlign: 'right' }}>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }} className="anim">
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr minmax(260px, 300px)', overflow: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ padding: '14px 16px', borderRight: `1px solid var(--c-bd)`, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              return (
                <div key={a.id} style={{ padding: '11px 12px', background: 'var(--c-sf2)', border: `1px solid var(--c-bd)`, borderRadius: 9, marginBottom: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: C.br }}>{a.name}</div>
                      <div style={{ fontSize: 10, color: C.mu }}>Lehrjahr {a.apprenticeship_year || 1} · {a.email}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 16, fontWeight: 800, color: C.ac, fontFamily: C.mono }}>{pct}%</div>
                      <div style={{ fontSize: 9, color: C.mu }}>Gesamt</div>
                    </div>
                  </div>
                  <ProgressBar value={pct} color={pct === 100 ? C.gr : C.ac} height={4} />
                  <div style={{ display: 'flex', gap: 8, marginTop: 7, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 10, color: C.mu }}>📁 {myProjects.length} Projekte</span>
                    ...
                    <span style={{ fontSize: 10, color: C.mu }}>{myTasks.length} offen</span>
                  </div>
                </div>
              );
            })}
            ...
            <div style={{ marginBottom: 14, flexShrink: 0 }}>
              ...
              {problems.map(p => (
                <button key={p.id} onClick={() => onOpenProject(p.id)} className="row-btn"
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: C.br, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.title}</div>
                    <div style={{ fontSize: 10, color: C.mu }}>{users.filter(u => p.assignees.includes(u.id)).map(u => u.name.split(' ')[0]).join(', ')}</div>
                  </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', overflowY: 'auto' }}>
          ...
          <div style={{ padding: '14px 16px', borderBottom: `1px solid var(--c-bd)`, flexShrink: 0 }}>
                ...
                {pending.slice(0, 4).map(r => (
                  <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderBottom: `1px solid var(--c-bd)22` }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 9, color: C.mu }}>KW {r.week_number} · {fmtDate(r.week_start)}</div>
                    </div>
                    <span className="tag" style={{ background: C.ywd, color: C.yw, border: `1px solid ${C.yw}30`, fontSize: 9 }}>Prüfen</span>
                  </div>
                ))}
                {pending.length > 4 && <div style={{ fontSize: 10, color: C.mu, marginTop: 6, textAlign: 'center' }}>+{pending.length - 4} weitere</div>}
              </>
            )}
          ...
          <div style={{ flexShrink: 0, padding: '10px 16px 12px', borderTop: `1px solid var(--c-bd)`, background: 'var(--c-sf)', maxHeight: '35%', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {(() => {
                ...
                if (sorted.length === 0) return <div style={{ fontSize: 11, color: C.mu, fontStyle: 'italic', padding: '4px 0' }}>Noch keine Aktivität</div>;
                return sorted.map((a, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '4px 0', borderBottom: `1px solid var(--c-bd)22` }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 11, color: C.tx, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.text}</div>
                      <div style={{ fontSize: 9, color: C.mu }}>{a.sub} · {fmt(a.date)}</div>
                    </div>
                  </div>
                ));
              })()}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, gap: 10, flexShrink: 0, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 7, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', left: 8, top: '50%', transform: 'translateY(-50%)', color: C.mu, pointerEvents: 'none' }}><IcoSearch size={12} /></div>
            ...
            <thead>
              <tr style={{ borderBottom: `2px solid var(--c-bd)`, textAlign: 'left' }}>
                {['Projekt','Status','Fortschritt','Azubis','Deadline','Aufgaben'].map(h => (
                  <th key={h} style={{ padding: '7px 12px', fontSize: 10, fontWeight: 700, color: C.mu, textTransform: 'uppercase', letterSpacing: .8, whiteSpace: 'nowrap' }}>{h}</th>
                ))}
                ...
                return (
                  <tr key={p.id}
                    ...
                    <td style={{ padding: '9px 12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <div>
                          ...
                          {p.description && <div style={{ fontSize: 10, color: C.mu, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 280 }}>{p.description}</div>}
                        </div>
                      </div>
                    </td>
                    ...
                    <td style={{ padding: '9px 12px', minWidth: 120 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                        <ProgressBar value={pct} color={pct === 100 ? C.gr : C.ac} height={4} />
                        <span style={{ fontSize: 10, fontFamily: C.mono, color: C.mu, flexShrink: 0 }}>{pct}%</span>
                      </div>
                    </td>
                    <td style={{ padding: '9px 12px' }}>
                      <div style={{ display: 'flex' }}>
                        {au.slice(0, 4).map((u, i) => <div key={u.id} style={{ marginLeft: i > 0 ? -6 : 0, zIndex: i }}><Avatar name={u.name} size={22} /></div>)}
                        {au.length > 4 && <div style={{ width: 22, height: 22, borderRadius: '50%', background: 'var(--c-bd2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: C.mu, marginLeft: -6 }}>+{au.length-4}</div>}
                      </div>
                    </td>
                    <td style={{ padding: '9px 12px', whiteSpace: 'nowrap', fontFamily: C.mono, fontSize: 11, color: over ? C.cr : C.mu, fontWeight: over ? 700 : 400 }}>
                      {p.deadline ? (over ? '⚠ ' : '') + fmtDate(p.deadline) : '–'}
                    </td>
                    <td style={{ padding: '9px 12px', whiteSpace: 'nowrap', fontFamily: C.mono, fontSize: 11, color: C.mu }}>
                      {done}/{p.tasks.length}
            ...
            {visible.map(p => {
              ...
              return (
                <article key={p.id} role="listitem" className="card proj-card"
                  ...
                  {p.description && (
                    <p style={{ fontSize: 12, color: C.mu, margin: '0 0 9px', lineHeight: 1.55, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{p.description}</p>
                  )}
                  <div style={{ display: 'flex', gap: 7, marginBottom: 9, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {lc > 0 && <span style={{ fontSize: 10, color: C.mu, display: 'flex', alignItems: 'center', gap: 3 }}><IcoLink size={10} />{lc}</span>}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: pct !== null ? 8 : 0 }}>
                    <div style={{ display: 'flex' }}>
                      ...
                      {au.length > 5 && <div style={{ width: 22, height: 22, borderRadius: '50%', background: 'var(--c-bd2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: C.mu, marginLeft: -6 }}>+{au.length - 5}</div>}
                    </div>
                    <span style={{ fontSize: 10, color: C.mu, fontFamily: C.mono }}>{done}/{p.tasks.length}</span>
                  </div>
        ...
        {archived.length > 0 && (
          <div style={{ marginTop: 28 }}>
            <button onClick={() => setShowArchive(s => !s)}
              style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'none', border: `1px solid var(--c-bd)`, borderRadius: 7, padding: '6px 14px', cursor: 'pointer', color: C.mu, fontSize: 12, fontWeight: 600, marginBottom: 12, transition: 'border-color .15s' }}
                ...
                {archived.map(p => (
                  <div key={p.id} style={{ background: 'var(--c-sf2)', border: `1px solid var(--c-bd)`, borderRadius: 9, padding: '11px 14px', opacity: .6 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 8 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: C.br, wordBreak: 'break-word', flex: 1 }}>{p.title}</span>
                      <span className="tag" style={{ background: 'var(--c-sf3)', color: C.mu, border: `1px solid var(--c-bd2)`, fontSize: 9, flexShrink: 0 }}>Archiv</span>
                    </div>
